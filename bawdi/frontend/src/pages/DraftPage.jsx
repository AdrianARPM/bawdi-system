// src/pages/DraftPage.jsx  — v7 (restricted: Admin, Verifikator, Approval)
import { useState, useEffect } from 'react';
import { Link, Navigate } from 'react-router-dom';
import {
  Archive, Search, Filter, X, FileText,
  Calendar, Truck, Loader, ShieldOff
} from 'lucide-react';
...
export default function DraftPage() {
  ...
  const resetFilter = () => { setFilterKendaraan(''); setFilterBulan(''); setFilterTahun(''); setPage(1); setQ(''); };
  ...
  const handleExportPDF = async () => {
    setExporting('pdf');
    try {
      toast.loading('Menyiapkan PDF...', { id: 'export' });
      const allData = await fetchAllForExport();
      if (!allData.length) { toast.error('Tidak ada data'); setExporting(''); return; }
      const filterInfo = {
        kendaraan: filterKendaraan,
        bulanLabel: bulanList.find(b => b.bulan === Number(filterBulan) && b.tahun === Number(filterTahun))?.label,
      };
      await exportToPDF(allData, filterInfo);
      toast.success(`${allData.length} data berhasil diekspor!`, { id: 'export' });
    } catch (err) { toast.error('Gagal: ' + err.message, { id: 'export' }); }
    setExporting('');
  };
  ...
  const handleExportSinglePDF = async (draftItem) => {
    ...
    setExporting(key);
    try {
      toast.loading('Membuat PDF...', { id: 'export-single' });
      const { data } = await submissionAPI.getOne(draftItem.id);
      await exportSinglePDF(data.data);
      toast.success('PDF berhasil dibuat!', { id: 'export-single' });
    } catch (err) { toast.error('Gagal: ' + err.message, { id: 'export-single' }); }
    setExporting('');
  };
    ...
    <div className="max-w-2xl mx-auto space-y-5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-0.5">Pengajuan selesai & ditutup</p>
        </div>
        <button onClick={handleExportPDF} disabled={!!exporting || loading}
          className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-red-500 hover:bg-red-600 disabled:opacity-50 text-white text-xs font-bold transition-all shadow-sm">
          {exporting === 'pdf' ? <Loader size={13} className="animate-spin"/> : <FileText size={13}/>}
          Export PDF
        </button>
      </div>
      ...
      <div className="bg-blue-50 border border-blue-100 rounded-xl px-4 py-2.5">
        <p className="text-xs text-blue-600 font-medium">
          ...
          Export PDF akan mengunduh semua data sesuai filter aktif.
        </p>
      </div>
          ...
          {filtered.map(d => (
            <div key={d.id} className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
              <Link to={`/submissions/${d.id}`} className="block p-4 hover:bg-slate-50 transition-colors">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-slate-500 truncate mb-1">{d.vendor_pilihan===2?d.vendor2:d.vendor}</p>
                  ...
                  <div className="text-right flex-shrink-0">
                    ...
                    {d.nota_url
                      ? <span className="inline-flex items-center gap-0.5 mt-1 text-[9px] font-bold bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-full"><FileText size={8}/> Nota ✓</span>
                      : <span className="inline-flex items-center gap-0.5 mt-1 text-[9px] font-bold bg-red-100 text-red-500 px-1.5 py-0.5 rounded-full"><FileText size={8}/> No Nota</span>
                    }
                  </div>
                </div>
              </Link>
              <div className="px-4 py-2 border-t border-slate-50 flex items-center justify-between bg-slate-50/50">
                ...
                <button onClick={() => handleExportSinglePDF(d)} disabled={exporting===`pdf-${d.id}`}
                  className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-red-500 hover:bg-red-600 disabled:opacity-50 text-white text-[10px] font-bold">
                  {exporting===`pdf-${d.id}` ? <Loader size={10} className="animate-spin"/> : <FileText size={10}/>}
                  PDF
                </button>
              </div>
            </div>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2 pb-4">
          <button onClick={() => setPage(p=>Math.max(1,p-1))} disabled={page===1}
            className="px-4 py-2 rounded-xl border border-slate-200 text-sm font-semibold text-slate-600 disabled:opacity-40">← Prev</button>
          {Array.from({length:Math.min(5,totalPages)},(_,i)=>{
            const p=page<=3?i+1:page+i-2; if(p<1||p>totalPages) return null;
            return <button key={p} onClick={()=>setPage(p)} className={`w-9 h-9 rounded-xl text-sm font-bold ${p===page?'bg-amber-500 text-white':'border border-slate-200 text-slate-600'}`}>{p}</button>;
          })}
          <button onClick={()=>setPage(p=>Math.min(totalPages,p+1))} disabled={page===totalPages}
            className="px-4 py-2 rounded-xl border border-slate-200 text-sm font-semibold text-slate-600 disabled:opacity-40">Next →</button>
        </div>
      )}
    </div>
  );
