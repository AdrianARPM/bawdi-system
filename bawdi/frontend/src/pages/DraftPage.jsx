...
import { Archive, Search, Filter, X, FileText, Calendar, Truck } from 'lucide-react';
...
export default function DraftPage() {
  ...
  // Filter lokal berdasarkan search text
    ...
    <div className="max-w-2xl mx-auto space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-black text-slate-800 flex items-center gap-2">
            <Archive size={20} className="text-slate-500"/> Draft Pengajuan
          </h1>
          <p className="text-sm text-slate-400 mt-0.5">Arsip pengajuan yang sudah selesai & ditutup</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <Card className="!p-4">
          ...
          <p className="text-xl font-black text-emerald-600">{fmtCurrency(stats.totalBayar)}</p>
          <p className="text-xs text-slate-400 mt-1">
            {filterBulan ? `bulan ${bulanList.find(b => b.bulan === Number(filterBulan) && b.tahun === Number(filterTahun))?.label || 'dipilih'}` : 'semua periode'}
          </p>
        </Card>
      </div>
      ...
      <Card className="!p-4">
        <div className="flex items-center gap-2 mb-3">
          ...
          <p className="text-sm font-bold text-slate-700">Filter Arsip</p>
          {hasFilter && (
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              <button
                onClick={() => { setFilterKendaraan(''); setPage(1); }}
                className={`px-3 py-1.5 rounded-full text-xs font-semibold border transition-all ${
                  !filterKendaraan ? 'bg-amber-500 border-amber-500 text-white' : 'bg-white border-slate-200 text-slate-500 hover:border-slate-300'
                }`}>
                Semua
              </button>
              {kendaraanList.map(k => (
                <button key={k}
                  onClick={() => { setFilterKendaraan(k); setPage(1); }}
                  className={`px-3 py-1.5 rounded-full text-xs font-semibold border transition-all ${
                    filterKendaraan === k ? 'bg-amber-500 border-amber-500 text-white' : 'bg-white border-slate-200 text-slate-500 hover:border-slate-300'
                  }`}>
                  {k}
                </button>
              ))}
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              <button
                onClick={() => { setFilterBulan(''); setFilterTahun(''); setPage(1); }}
                className={`px-3 py-1.5 rounded-full text-xs font-semibold border transition-all ${
                  !filterBulan ? 'bg-blue-500 border-blue-500 text-white' : 'bg-white border-slate-200 text-slate-500 hover:border-slate-300'
                }`}>
                Semua Bulan
              </button>
              {bulanList.map(b => (
      ...
      {!loading && (
        <div className="flex items-center justify-between">
          ...
          {total > LIMIT && (
            <p className="text-xs text-slate-400">Halaman {page} / {totalPages}</p>
          )}
        </div>
      )}
      ...
      {loading ? (
        ...
        <Empty icon={Archive} message="Belum ada arsip pengajuan" sub="Arsip muncul setelah pengajuan ditutup oleh Approval"/>
      ) : (
        <div className="space-y-2.5">
          {filtered.map(d => (
            <Link key={d.id} to={`/submissions/${d.id}`}
              className="block bg-white rounded-2xl p-4 border border-slate-100 shadow-sm transition-all hover:shadow-md hover:-translate-y-0.5">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-slate-500 truncate mb-1">
                    ...
                    {d.vendor_pilihan && <span className="ml-1 text-[9px] text-emerald-500">(Vendor {d.vendor_pilihan} dipilih)</span>}
                  </p>
                  ...
                  <div className="flex items-center gap-3 mt-1.5">
                    {d.ditutup_at && (
                      <span className="text-[10px] text-slate-400 flex items-center gap-0.5">
                        <span>🏁</span> Ditutup: {fmtDate(d.ditutup_at)}
                      </span>
                    )}
                    {d.tanggal_bayar && (
                      <span className="text-[10px] text-emerald-500 font-semibold flex items-center gap-0.5">
                        <span>💰</span> Bayar: {fmtDate(d.tanggal_bayar)}
                      </span>
                    )}
                  </div>
                </div>
                ...
                <div className="text-right flex-shrink-0">
                  ...
                  <p className="text-[10px] text-slate-400 mt-0.5">
                    {d.jumlah_bayar > 0 ? 'Dibayar' : 'Total'}
                  </p>
                  {d.nota_url && (
                    <span className="inline-flex items-center gap-0.5 mt-1.5 text-[9px] font-bold bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-full">
                      <FileText size={8}/> Nota ✓
                    </span>
                  )}
                </div>
              </div>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2 pb-4">
          <button onClick={() => setPage(p => Math.max(1, p-1))} disabled={page === 1}
            className="px-4 py-2 rounded-xl border border-slate-200 text-sm font-semibold text-slate-600 disabled:opacity-40 hover:bg-slate-50 transition-colors">
            ← Prev
          </button>
