  ...
  return (
    <div className="max-w-2xl mx-auto space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-black text-slate-800">Manajemen Pengguna</h1>
      ...
      {/* Form Tambah User */}
      {showForm && (
        <Card className="animate-fade-in border-amber-200">
          <div className="px-4 py-3 border-b border-slate-50">
            <p className="text-sm font-bold text-slate-700">Tambah Pengguna Baru</p>
            <p className="text-xs text-slate-400 mt-0.5">Password default = NIK. Karyawan wajib ganti saat login pertama.</p>
          </div>
          <form onSubmit={handleAdd} className="p-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-bold text-slate-600 mb-1.5">Role / Jabatan Sistem *</label>
                <select value={form.role} onChange={e => set('role', e.target.value)}
                  className="w-full px-3 py-2.5 rounded-xl border border-slate-200 text-sm text-slate-800 outline-none focus:border-amber-400 focus:ring-2 focus:ring-amber-100">
      ...
      <Card padding={false}>
        {users.map((u, i) => (
          <div key={u.id}
            className={`flex items-center gap-3 px-4 py-3.5 ${i < users.length - 1 ? 'border-b border-slate-50' : ''} ${!u.is_active ? 'opacity-50' : ''}`}>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {!u.is_active && <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-slate-100 text-slate-500">Nonaktif</span>}
              </div>
              ...
              {u.last_login && (
                <p className="text-[10px] text-slate-300 mt-0.5">Login terakhir: {fmtDate(u.last_login)}</p>
              )}
            </div>
            ...
            <div className="flex flex-col gap-1.5 flex-shrink-0">
              <button
                ...
                {u.is_active
                  ...
                  : <><ToggleLeft  size={16} className="text-slate-400"  /> Nonaktif</>}
              </button>
              <button
                onClick={() => handleReset(u.id, u.name, u.nik)}
                title="Reset password ke NIK"
                className="flex items-center gap-1 text-[10px] font-semibold text-amber-500 hover:text-amber-700 transition-colors">
      ...
      <div className="bg-blue-50 border border-blue-100 rounded-2xl p-4">
        <p className="text-xs font-bold text-blue-700 mb-2">📋 Panduan Role Pengguna</p>
