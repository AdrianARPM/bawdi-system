    ...
    <div className="space-y-5 max-w-2xl mx-auto">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">Selamat datang, {user?.name}</p>
        </div>
        ...
        <div className="bg-orange-50 border border-orange-200 rounded-2xl p-4">
          <div className="flex gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-orange-600 mb-2">Segera hubungi Verifikator atau Approval terkait:</p>
              {stats.alerts.map(a => (
                <button key={a.id} onClick={() => navigate(`/submissions/${a.id}`)}
                  className="block text-xs font-semibold text-orange-600 underline mb-1 text-left">
      ...
      <div className="grid grid-cols-2 gap-3">
        <StatCard label="Hari Ini"    value={stats?.today ?? 0}               icon={FileText}     iconBg="bg-blue-50"     iconColor="text-blue-600" />
        <StatCard label="Dalam Proses" value={(stats?.menunggu_verifikasi ?? 0) + (stats?.terverifikasi ?? 0)} icon={Clock} iconBg="bg-amber-50" iconColor="text-amber-600" />
        <StatCard label="Disetujui"   value={stats?.disetujui ?? 0}           icon={CheckCircle}  iconBg="bg-emerald-50"  iconColor="text-emerald-600" />
        <StatCard label="Ditolak"     value={stats?.ditolak ?? 0}             icon={XCircle}      iconBg="bg-red-50"      iconColor="text-red-600" />
      </div>

      {/* Recent submissions */}
      <Card padding={false}>
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-50">
          <p className="text-sm font-bold text-slate-700">Pengajuan Terbaru</p>
          <Link to="/submissions" className="text-xs font-semibold text-brand-500 hover:text-brand-600">Lihat Semua →</Link>
        </div>
        {recent.length === 0 && (
          <div className="py-10 text-center text-slate-400 text-sm">Belum ada pengajuan</div>
        )}
        {recent.map((s, i) => {
          const isAlert = ['Menunggu Verifikasi','Terverifikasi'].includes(s.status) && daysSince(s.tanggal) > 3;
          return (
            <Link key={s.id} to={`/submissions/${s.id}`}
              className={`flex items-center gap-3 px-4 py-3 hover:bg-slate-50 transition-colors ${i < recent.length-1 ? 'border-b border-slate-50' : ''}`}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-400 truncate">{s.kendaraan} · {s.vendor}</p>
              </div>
