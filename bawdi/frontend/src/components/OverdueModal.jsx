          ...
          <div className="space-y-2.5">
            {items.map((it) => (
              <button
                ...
                className="w-full text-left bg-slate-50 hover:bg-amber-50 border border-slate-200 hover:border-amber-300 rounded-2xl p-3.5 transition-all group">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-xs font-black text-slate-800 truncate">{it.nomor_pengajuan}</span>
                      <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${it.type==='PAR'?'bg-purple-100 text-purple-600':'bg-blue-100 text-blue-600'}`}>
                        {it.type}
                      </span>
                    </div>
                    <p className="text-[11px] text-slate-500 truncate">{it.kendaraan} • {it.pemohon?.name}</p>
                    <div className="flex items-center gap-2 mt-1.5">
                      <span className="inline-flex items-center gap-1 text-[10px] font-bold text-red-500">
                        <Clock size={10}/> {it.days} hari
                      </span>
                      <span className="text-[10px] text-amber-600 font-semibold">{it.action}</span>
                      {it.total_harga > 0 && (
                        <span className="text-[10px] text-slate-400">{fmtRpShort(it.total_harga)}</span>
                      )}
                    </div>
                  </div>
                  <ArrowRight size={16} className="text-slate-300 group-hover:text-amber-500 transition-colors flex-shrink-0 mt-1"/>
                </div>
              </button>
            ))}
          </div>
